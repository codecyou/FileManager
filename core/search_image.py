...
def deal_image(img1_path, img2_path, search_path, dst_path, same_path):
    ...
    if img1_path not in old_new_record:
        ...
        mutex.release()
        # 拼接新路径
        new_img1_name = os.path.basename(Mytools.make_new_path(img1_path, search_path, same_path, name_simple=False))
    ...
    if img2_path not in old_new_record:
        ...
        new_img2_name = os.path.basename(Mytools.make_new_path(img2_path, dst_path, same_path, name_simple=False))
...
def init():
    ...
    :return:
    """
    global g_img_list, g_dst_img_list, new_old_record, old_new_record
    g_img_list = []
    g_dst_img_list = []
    new_old_record = {}
    old_new_record = {}
...
def export_phash_info(export_path):
    ...
    with open(export_path, 'w', encoding='utf-8') as f:
        json.dump(g_dst_img_list, f, ensure_ascii=False)  # 记录配置信息
